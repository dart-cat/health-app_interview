...
import 'package:vstrecha/presentation/widgets/event_type_selector.dart';
...
class _CalendarEventPageState extends State<CalendarEventPage> {
  ...
  void _removeItem(CreateEventLoaded state) {
    if (state.event != null) {
      BlocProvider.of<CalendarBloc>(context)
          .add(RemoveCalendarEvent(event: state.event!));
    }
  }

  @override
  Widget build(BuildContext context) {
    final CreateEventBloc bloc = BlocProvider.of<CreateEventBloc>(context);
                  ...
                  child: ListView(
                    ...
                    children: [
                      Padding(
                        ...
                        child: EventTypeSelector(value: state.title),
                      ),
                      Padding(
                        ...
                        child: MainInput(
                          hint: 'required-information'.i18n(),
                          label: 'note'.i18n(),
                          isRequired: false,
                          value: state.description,
                          onChanged: (value) =>
                              bloc.add(SetEventDescription(description: value)),
                        ),
                      ),
                      Padding(
                        padding: const EdgeInsets.only(
                        ...
                        child: DatePicker(
                          label: 'event-start-date'.i18n(),
                          value: state.startDate,
                          onChanged: (value) =>
                              bloc.add(SetEventStartDate(startDate: value)),
                          isRequired: false,
                        ),
                      ),
                      Padding(
                        ...
                        child: DatePicker(
                          label: 'event-end-date'.i18n(),
                          value: state.endDate,
                          onChanged: (value) =>
                              bloc.add(SetEventEndDate(endDate: value)),
                          isRequired: false,
                        ),
                      ),
                      Padding(
                        ...
                        child: DropDown(
                          ...
                          onChanged: (value) {
                            ...
                            switch (index) {
                              case 0:
                                bloc.add(const SetEventFrequency(
                              ...
                              child: MainButton(
                                onTap: () {
                                  ...
                                  CalendarBloc cal =
                                      BlocProvider.of<CalendarBloc>(context);
                                  if (state.event != null) {
                                    cal.add(UpdateCalendarEvent(
                                      oldEvent: state.event!,
                                      newEvent: CalendarEventItem(
                                        title: state.title,
